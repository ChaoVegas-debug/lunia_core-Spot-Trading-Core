...
export const SystemStateWidget: React.FC = () => {
  ...
  const ops = usePolledResource<OpsState>((signal) => getOpsState(signal, client), 2500, [auth.role, auth.tenantId]);
  const status = usePolledResource<StatusSnapshot>((signal) => getStatus(signal, client), 4000, [auth.role, auth.tenantId]);
  const health = usePolledResource((signal) => getHealth(signal, client), 7000, [auth.role, auth.tenantId]);
