...
export const AuthProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    setAuthState((prev) => ({
      ...
      role: resp.role,
      user:
        userProfile ?? { id: resp.user_id, email, role: resp.role, is_active: true, created_at: '', last_login_at: resp.expires_at, tenant_id: resp.tenant_id },
      tenantId: resp.tenant_id ?? userProfile?.tenant_id ?? prev.tenantId ?? null,
      expiresAt: resp.expires_at
    }));
  };

  const setTenantId = (tenantId: string | null) => {
    setAuthState((prev) => ({ ...prev, tenantId }));
  };
